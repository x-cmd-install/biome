# biome

[English version](./README.md)

A toolchain for web projects, aimed to provide functionalities to maintain them. Biome offers formatter and linter, usable via CLI and LSP.

[![x-cmd/install — biome Code Quality Monitoring Repo Card](https://x-cmd.com/repo-card/biome.svg?lang=zh)](https://x-cmd.com/install/biome)

## 安装

```sh
x install biome
```

## 代码洞察

合计: **1,221,051** 行代码（覆盖前 5 种语言、共 **11370** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| Rust | 968,321 | 19,929 | 75,226 | 4739 |
| TypeScript | 66,616 | 12,539 | 6,405 | 1811 |
| Json | 60,135 | 0 | 36 | 1320 |
| JavaScript | 52,407 | 5,389 | 8,578 | 2758 |
| Css | 17,087 | 1,081 | 3,526 | 742 |

## 源代码

- **上游仓库**: <https://github.com/biomejs/biome>
- **官网**: <https://biomejs.dev>
- **许可证**: Apache-2.0

## 发布

- **最新版本**: `@biomejs/biome@2.5.14` (2026-09-16)
- **最近提交**: 2026-09-23
- **Release 含资产**: 8 个

## 流行度

- **Star**: 25,852 · **Fork**: 1,242 · **开放 issue**: 3,531 · **贡献者**: 759

## 累计统计

- **发布数**: 160 · **已合并 PR**: 6275 · **开放 PR**: 115 · **已关闭 issue**: 3262 · **开放 issue**: 269 · **提交数**: 10977

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-25 | 4 | 238 | 32 | 60 | 26 | 285 |
| last60d | 2026-07-26 | 9 | 461 | 58 | 138 | 46 | 535 |
| 90d | 2026-06-26 | 13 | 607 | 67 | 190 | 65 | 740 |
| last180d | 2026-03-28 | 23 | 1240 | 93 | 379 | 125 | 1640 |
| 360d | 2025-09-29 | 53 | 2237 | 112 | 953 | 191 | 3149 |
| last720d | 2024-10-04 | 90 | 3921 | 115 | 2029 | 245 | 4208 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [biome-darwin-arm64](https://github.com/biomejs/biome/releases/download/@biomejs/biome@2.5.14/biome-darwin-arm64) | 54.3 MiB | `native/darwin/arm64` |
| [biome-darwin-x64](https://github.com/biomejs/biome/releases/download/@biomejs/biome@2.5.14/biome-darwin-x64) | 56.6 MiB | `native/darwin/x64` |
| [biome-linux-arm64](https://github.com/biomejs/biome/releases/download/@biomejs/biome@2.5.14/biome-linux-arm64) | 56.2 MiB | `native/linux/arm64` |
| [biome-linux-arm64-musl](https://github.com/biomejs/biome/releases/download/@biomejs/biome@2.5.14/biome-linux-arm64-musl) | 53.3 MiB | `native/linux/arm64/musl` |
| [biome-linux-x64](https://github.com/biomejs/biome/releases/download/@biomejs/biome@2.5.14/biome-linux-x64) | 61.7 MiB | `other` |
| [biome-linux-x64-musl](https://github.com/biomejs/biome/releases/download/@biomejs/biome@2.5.14/biome-linux-x64-musl) | 61.2 MiB | `other` |
| [biome-win32-arm64.exe](https://github.com/biomejs/biome/releases/download/@biomejs/biome@2.5.14/biome-win32-arm64.exe) | 62.4 MiB | `native/win/arm64` |
| [biome-win32-x64.exe](https://github.com/biomejs/biome/releases/download/@biomejs/biome@2.5.14/biome-win32-x64.exe) | 72.6 MiB | `native/win/x64` |

## 改进这些数据

biome 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `biome` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/biome.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260924.yml` · 2026-09-24T04:40:43Z._
